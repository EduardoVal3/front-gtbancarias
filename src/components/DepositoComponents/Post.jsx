...
import { createDeposito } from "../../services/depositoService";
...
const PostDeposito = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notify("Depósito realizado exitosamente", "success", 4000);
      form.resetValues();
      setFormData({ cuentaId: 0, monto: 0, hechoPor: "", descripcion: "", cuentaDetalle: "" });
    } catch (error) {
      console.error("❌ Error al realizar el depósito:", error);
      notify("Error al registrar el depósito", "error", 4000);
    }
  };
  ...
  return (
    <Container theme={theme}>
      <FormWrapper>
        <Title theme={theme}>Registrar Depósito</Title>
        <ResponsiveForm
          ref={formRef}
          formData={formData}
        ...
        >
          ...
          <SimpleItem dataField="hechoPor" label={{ text: "Hecho por" }}>
            <RequiredRule message="Debe indicar quién realiza el depósito." />
          </SimpleItem>
      ...
      <PreviewPanel theme={theme}>
        ...
        <p><span>Monto depósitado:</span> {`$${formData.monto.toFixed(2)}`}</p>
