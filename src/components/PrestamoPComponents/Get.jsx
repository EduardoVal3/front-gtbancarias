...
const GetPrestamosP = () => {
  ...
  useEffect(() => {
    const fetchClientes = async () => {
      try {
        setIsLoading(true);
        const data = await getPrestamosPersonales();
        setClientes(data);
        setIsLoading(false);
      } catch (err) {
  ...
  const onExporting = useCallback((e) => {
    ...
    const worksheet = workbook.addWorksheet('PréstamosPersonales');
    ...
    exportDataGrid({
      ...
      workbook.xlsx.writeBuffer().then((buffer) => {
        saveAs(new Blob([buffer], { type: 'application/octet-stream' }), 'PréstamosPersonales.xlsx');
      });
    });
